...
colnames(model.fl.df) <- c("$r^2$", "$r^2_{adj}$", "$C_p$")
kable(model.fl.df)
```
...
The standard errors of coefficients are underestimated due to the high serial correlation. Instead the Newey-West standard errors, both Heteorscedasticity and Autocorrelation (HAC), should be used instead [[1]](#references).
...
We cannot do a cross-validation on a time series data. We instead train our model on the first 80% of the data, and test it on the last 20% of the data. But this may not be a valid way to evaluate the model involving a time series. The TODO: why not?
The root mean square prediction errors are used to evaluate the three models.
...
The real NHPI for June 2022 was 125.9 [[4]](#references).
